...
1. **Apple Developer portal** → Identifiers → CloudKit Containers →
   **+ New**. Container ID: `iCloud.com.jimzucker.iInteract` (matches
...
2. **Xcode** → iInteract target → Signing & Capabilities → **+
   ...
   <array>
       <string>iCloud.com.jimzucker.iInteract</string>
   </array>
...
struct LiveCloudKitDatabase: CloudKitDatabase {
    let database = CKContainer(identifier: "iCloud.com.jimzucker.iInteract")
